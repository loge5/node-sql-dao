...
class DatabaseAccessObject extends Model {
  ...
  async insert (transaction = undefined) {
    ...
    for (let relation of this.constructor.getRelations()) {
      await this._insertRelationBefore(relation, transaction)
    }
    ...
    for (let relation of this.constructor.getRelations()) {
      await this._insertRelationAfter(relation, transaction)
    }
   ...
   * @param {*} transaction
   */
  async _insertRelationBefore (relation, transaction = undefined) {
    switch (true) {
      ...
      case relation instanceof RelationBelongsTo:
        return this._insertAttributeHasX(relation, transaction)
      case relation instanceof RelationManyMany:
        return this._insertAttributeManyManyBefore(relation, transaction)
      default:
   ...
   * @param {*} transaction
   */
  async _insertAttributeHasX (relation, transaction = undefined) {
    if (typeof this[relation.thisAttribute] !== 'object' || this[relation.thisAttribute] === null) {
      return
    }
    if (typeof this[relation.thisAttribute][relation.refKey] === 'undefined') {
      await this[relation.thisAttribute].insert(transaction)
    }
    if (typeof this[relation.thisAttribute][relation.refKey] === 'undefined') {
      throw new Error('cant insert new DAO')
    }
    this[relation.thisKey] = this[relation.thisAttribute][relation.refKey]
  }
   ...
   * @param {*} transaction
   */
  async _insertAttributeManyManyBefore (relation, transaction = undefined) {
    ...
    for (let refModel of this[relation.thisAttribute]) {
      if (refModel[relation.refKey] === 'undefined') {
        await this[relation.thisAttribute].insert(transaction)
      }
      if (typeof refModel[relation.refKey] === 'undefined') {
        throw new Error('cant insert new DAO')
      }
    }
  }
   ...
   * @param {*} transaction
   */
  async _insertRelationAfter (relation, transaction = undefined) {
    switch (true) {
      case relation instanceof RelationManyMany:
        return this._insertAttributeManyManyAfter(relation, transaction)
      case relation instanceof RelationHasOne:
      case relation instanceof RelationHasMany:
      case relation instanceof RelationBelongsTo:
        return
   ...
   * @param {*} transaction
   */
  async _insertAttributeManyManyAfter (relation, transaction = undefined) {
    if (!Array.isArray(this[relation.thisAttribute]) || this[relation.thisAttribute].length === 0) {
      return
    }
    let connection = this.constructor.getDatabaseConnection()
