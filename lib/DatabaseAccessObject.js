...
class DatabaseAccessObject extends Model {
  ...
  static async _setRelationAttributeManyMany (model, relation) {
    ...
    joins.push(new Join(
      ...
      [
        relation.relTableName,
        relation.relReferencedKey,
        relation.referencedClass.getTableName(),
        relation.referencedAttributeNameKey
      ]
    ))
    joins.push(new Join(
    ...
    let refModels = await relation.referencedClass.find(whereClause, joins)
    model[relation.attributeNameObject] = refModels
  }
