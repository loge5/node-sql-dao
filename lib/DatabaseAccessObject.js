...
class DatabaseAccessObject extends Model {
  ...
  /**
   * Creates object with only attributes
   * (removes all DAO specific properties)
   * @param {number} maxDepth for recursion
   */
  toPlainObject (maxDepth = 1000) {
    const obj = {}
    const attributes = this.constructor.getAttributeNames()
    this.constructor.getRelations().forEach(rel => attributes.push(rel.thisAttribute))
    for (const attr of attributes) {
      obj[attr] = this._plainValue(this[attr], maxDepth)
    }
    return obj
  }
   ...
   * @returns {*}
   */
  _plainValue (val, maxDepth, depth = 1) {
    if (depth === maxDepth) {
      throw new Error('max depth of recursion reached')
    }
    if (val instanceof DatabaseAccessObject) {
      return val.toPlainObject()
    } else if (Array.isArray(val)) {
      return val.map(v => this._plainValue(v, depth + 1, maxDepth))
    } else {
      return val
    }
  }
